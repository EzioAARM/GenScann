 ...
 */
public final class CodificarCSharp {
    // Globales de la clase CodificarCSharp
    FileWriter escritor = null;
    // Contenido por defecto
    String[] Usings = { "using System;", "using System.Collections.Generic;", "using System.Linq;",
    "using System.Text;", "using System.Threading.Tasks;", "using System.IO;", "\n"};
    ...
    String FuncionLeerCaracter = "        "
            ...
            "            else\n" +
            "            {\n" +
            "                char car = Convert.ToChar(caracter);\n" +
            "                return car.ToString();\n" +
            "            }\n" +
            "        }";
    String FuncionLeerPalabra = "        "
            + "public String leerPalabra(String caracter)\n" +
            "        {\n" +
            "            String palabra = \"\";\n" +
            ...
            "        }";
    String Constructor = "        "
            + "public GenScann(string DireccionArchivoEntrada)\n" +
            "        {\n" +
            "            lector = new StreamReader(DireccionArchivoEntrada);\n" +
            "            ArchivoEntrada = DireccionArchivoEntrada;\n" +
            "            String[] ArchivoS = ArchivoEntrada.Split('.');\n" +
            "            for (int i = 0; i < ArchivoS.Length - 1; i++)\n" +
            "            {\n" +
            "                ArchivoSalida += ArchivoS[i];\n" +
            "            }\n" +
            "            ArchivoSalida += \"_output.txt\";\n" +
            "            Comparador();\n" +
            "        }";
    String FuncionVerificarConjunto = "        "
            + "public bool VerificarConjunto(String[] Conjunto, String caracter)\n" +
            "        {\n" +
            "            for (int i = 0; i < Conjunto.Length; i++)\n" +
            "            {\n" +
            "                if (caracter.Equals(Conjunto[i]))\n" +
            "                {\n" +
            "                    return true;\n" +
            "                }\n" +
            "            }\n" +
    ...
    public CodificarCSharp(String ubicacion) throws IOException
    {
        escritor = new FileWriter(new File(ubicacion + "Codificado.cs"));
        GenerarConjuntos();
        GenerarAcciones();
                ...
                for (k = 0; k < estadosTemp.size(); k++)
                {
                    if (k == 0)
                    {
                        ...
                        else
                        {
                            ...
                            ifGenerado += "\n" + tabs + tabs + tabs;
                            ifGenerado += "\tEstado = " + estadosTemp.get(k).Estado.Nombre + ";";
                            if (estadosTemp.get(k).Estado.Aceptacion)
                            {
                                ifGenerado += "\n" + tabs + tabs + tabs;
                                ifGenerado += "\tif (posicion == palabra.Length - 1)";
                                ifGenerado += "\t" + tabs + tabs + tabs;
                                ifGenerado += "\t{";
                                ifGenerado += "\n" + tabs + tabs + tabs + tabs;
                                ifGenerado += "return true;";
                                ifGenerado += "\n" + tabs + tabs + tabs;
